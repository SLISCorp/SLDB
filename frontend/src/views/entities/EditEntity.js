...
const EditEntity = ({ history, ...props }) => {
  ...
  const getEntityDetails = () => {
    Http("GET", apiPath.getEntityDetails + "/" + props.match.params.id)
      .then((res) => {
        res = res.data;
        if (res.status) {
          setEntityData(res.data);
          setProperties(
            typeof res.data.properties == "object" ? res.data.properties : {}
          );
        } else {
          showDangerToast(res.message);
        }
  ...
  const onSubmit = (data) => {
    ...
    if (Object.keys(properties).length == 0) {
      ...
      Http(
        "PUT",
        apiPath.addEntity + "/" + props.match.params.id,
        JSON.stringify(body)
      )
        .then((res) => {
          res = res.data;
  ...
  const setValue = (
    ...
    if (properties.length > 1) {
      ...
      console.log("obj[properties[0]] -->", JSON.stringify(obj[properties[0]]));
      if (isArrayItem) {
  ...
  const _renderPropertyViewer = (val, key, margin = 0) => {
    ...
    return (
      <Fragment key={key + 1}>
        ...
        <CCol xs="4" md="4" size="md" className="text-right">
          {btnVisible &&
            ((val.data_type == "Array" && val.items && !val.items.data_type) || val.data_type == "Object") && (
              <span
                style={{ cursor: "pointer" }}
                onClick={(e) => {
                  setBtnVisible(false);
