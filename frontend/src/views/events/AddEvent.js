...
const AddEvent = ({ match, }) => {
  ...
  const [firstEntityId, setFirstEntityId] = useState("");
  ...
  const SysyemEventArray = ["_Group", "_User", "_Schema", "_Event", "_Permission"];
  ...
  const handleChange = (evt, onlyNumber = false, trim = false) => {
    ...
    if (evt.target.name == 'event_type' & evt.target.value == 'DATA') {
      setSelectedEntityId(firstEntityId);
    } else if (evt.target.name == 'event_type' & evt.target.value == 'SYSTEM') {
      setSelectedEntityId('_Group');
    }
  };
  ...
  const getEntityData = (page = 1) => {
    Http("GET", apiPath.entitiesList)
      .then((res) => {
        res = res.data;
        if (res.status) {
          setFirstEntityId(res.data ? res.data[0]._id : '');
          setSelectedEntityId(res.data ? res.data[0]._id : '');
  ...
  const onSubmit = () => {
    ...
    let body = {};
    body["event_owner"] = formDetail.event_owner;
    body["event_type"] = formDetail.event_type;
    body["event_name"] = formDetail.event_name;
    ...
    Http("POST", apiPath.addEvent, JSON.stringify(body))
      .then((res) => {
        res = res.data;
        disabled = 0;
        console.log(res);
              ...
              <CCardBody>
                <CRow>
                  ...
                  <CFormGroup className={"col-md-6 col-xs-6 float-left"}>
                    ...
                    >
                      {formDetail.event_type == 'DATA' && EntityList.map((ele, ind) => {
                        return <option key={ind} value={ele._id}>{ele.name}</option>;
                      })}

                      {formDetail.event_type == 'SYSTEM' && SysyemEventArray.map((ele, ind) => {
