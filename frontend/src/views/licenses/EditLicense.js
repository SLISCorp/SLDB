...
const EditLicense = (props) => {
  ...
  const onSubmit = () => {
    ...
    Http("PUT", apiPath.editLicense + props.match.params.id, JSON.stringify(body))
      .then((res) => {
        res = res.data;
        disabled = 0;
        console.log(res);
  ...
  const getData = () => {
    Http("GET", apiPath.viewLicense + "/" + props.match.params.id)
      .then((res) => {
        res = res.data;
        if (res.status) {
          showToast(res.message);
              ...
              <CCardBody>
                ...
                <CRow>
                  <CFormGroup className={"col-md-6 col-xs-6 float-left"}>
                    ...
                    <DatePicker selected={new Date()} className="form-control col-md-12" placeholderText="Start Date"
                      dateFormat="dd/MM/yyyy"
                      minDate={formDetail.start_date == '' ? new Date() : formDetail.start_date}
                      onChange={(e) => setForm({ start_date: e })}
                      peekNextMonth
                      showMonthDropdown
                      showYearDropdown
                      dropdownMode="select"
